...
2. **Call `defineComponents(IgcDockManagerComponent)` in `main.ts`** before `bootstrapApplication` — without this the `<igc-dockmanager>` element renders as an unknown element
